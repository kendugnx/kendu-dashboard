...
BUY_BOT_POLL_MS=15000
BUY_BOT_CHAIN_DELAY_MS=1000
BUY_BOT_STATE_FILE=.buy-bot-state.json
...
The worker stores its last-seen trade IDs in `BUY_BOT_STATE_FILE`. On hosts with ephemeral disks, attach a persistent volume or replace the state file with a database-backed store before using it for production.
